...
expiry_date = datetime.datetime(2021, 3, 15, 23, 59, 59)
...
def get_data():
	...
	try:
		...
		row_items=[json_data['blockNum'], json_data['safeLow'], json_data['average'], json_data['fast'], json_data['fastest'], json_data['block_time'], json_data['speed'], json_data['safeLowWait'], json_data['avgWait'], json_data['fastWait'], json_data['fastestWait']]
		...
		row_items.append('ethgasstation')

		return row_items

	except:
		with open("ethgasstation_err.log", "a") as file:
...
def export_data(data, addheader=False): # by default False
	with open("ethgasstation_.csv", "a") as file:
		# if addheader=True and the csv does not have a header, add it
		if addheader and os.stat('ethgasstation_.csv').st_size<1:
			file.write('blockNum,safeLow,average,fast,fastest,block_time,speed,safeLowWait,avgWait,fastWait,fastestWait,datetime_utc,unixtime,date_utc,time_utc,record_date_utc,record_time_utc,date-GMT,url,sourceoracle\n')
		# export data to csv
